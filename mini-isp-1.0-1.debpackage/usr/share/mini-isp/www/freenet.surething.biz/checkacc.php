...
function rad_user_exists($username,$configValues){
  ...
  mysql_select_db($configValues['CONFIG_DB_NAME']) or die( "Unable to select database in rad_user_exists"); 
...
function rad_add_user($username,$password,$expiredate,$configValues){
  ...
  $con = mysql_connect($configValues['CONFIG_DB_HOST'],$configValues['CONFIG_DB_USER'],$configValues['CONFIG_DB_PASS'],true);
  mysql_select_db($configValues['CONFIG_DB_NAME']) or die( "Unable to select database in rad_add_user"); 
...
function rad_update_user($username,$password,$expiredate,$configValues){
  ...
  if (rad_user_exists($username,$configValues)>0){
    ...
    $con = mysql_connect($configValues['CONFIG_DB_HOST'],$configValues['CONFIG_DB_USER'],$configValues['CONFIG_DB_PASS'],true);
    mysql_select_db($configValues['CONFIG_DB_NAME']) or die( "Unable to select database in rad_update_user"); 
    $newexpire = date("d M Y",$expiredate);
...
function new_mysql($mysql_username,$mysql_password,$mysql_database,$mysql_host){
  ...
  $mysql = array('username'=>$mysql_username,
                 ...
                 'host'=>$mysql_host);
  $con = mysql_connect($mysql['host'],$mysql['username'],$mysql['password'],true);
  if (!$con)
  {
    die('Could not connect mysql in checkacc.php: ' . mysql_error());
  }
 
  mysql_select_db($mysql['database'],$con) or die( "Unable to select database in checkacc.php");
  //@mysql_select_db($mysql['database'],$con);
  return $con;
}


function get_query($query,$con){  
  $result=mysql_query($query,$con);
...
function checkacc($customers_email,$customers_password,$username,$password,$database,$configValues){
  ...
  $expire = update_account($customers_email,$cust_ip_address,$mysql,$configValues);
